...
class Game_2048():
    ...
    def possible_action(self):
        ...
        for i in range(3):
            for j in range(3):
                if(self.grid[i][j] == self.grid[i + 1][j] or
        ...
        while(self.status):
            ...
            if self.status is False:
                ...
                if self.score > 730:  # empirical mean see Algo_play
                    print('you did better than random IA')
                else:
                    print('you did worse than random IA')
    ...
    def clockwise_2048(self):
        """ Launches an AI game with clockwise movement
         (starting movement is randomized).
        """
        self.newcell_start()
        self.newcell()
        x = random.choice(self.directions)
    ...
    def adjacent_2048(self):
        """ Launches an AI game with only a first random movement
         and one of its adjacent ones.
        """
        ...
        x_adj = random.choice([self.directions[self.directions.index(x0) - 3],
                              self.directions[self.directions.index(x0) - 1]])
        x = x_adj
        block_game = 0
...
class Game_6561():
    ...
    def possible_action(self):
        ...
        for i in range(2):
            for j in range(2):
                if(self.grid[i][j] == self.grid[i + 1][j] or self.grid[i][j] ==
                   self.grid[i][j + 1]):
                    p_a = True
